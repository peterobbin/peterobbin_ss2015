...
float scope(float x, float y, float x2, float y2, float scale, float blurriness){
    ...
    pct = max(distance(st,vec2(x, y)),distance(st,vec2(x, y))) * scale;
    pct *= min(1.0 - distance(st,vec2(x2, y2)),1.0 - distance(st,vec2(x2, y2))) * 6.0 * scale;
...
void main(){
    ...
    float sinC = sin(u_time ) * 0.2 + 0.2;
    float cosC = cos(u_time ) * 0.2 + 0.2;
    float sinC2 = sin(u_time  + 2.0) * 0.2 + 0.2;
    float cosC2 = cos(u_time  + 2.0) * 0.2 + 0.2;
    float sinC3 = sin(u_time  + 4.0) * 0.2 + 0.2;
    float cosC3 = cos(u_time  + 4.0) * 0.2 + 0.2;
    ...
    vec3 circleMask = vec3(scope(sinC + 0.3 , cosC + 0.3 , cosC3  +0.3, sinC3  + 0.3 ,1.0 , 0.8 ));
    color = mix(color, vec3(0.5 + 0.5 , 0.0, 0.0), circleMask);

    vec3 circle2Mask = vec3(scope(sinC2 + 0.3 , cosC2 + 0.3 , cosC + 0.3 , sinC  + 0.3,1.0, 0.8 ));
    vec3 colorC2 = mix(color, vec3(0.0 , 1.0, 0.0), circle2Mask);
    color += colorC2;

    vec3 circle3Mask = vec3(scope(sinC3 + 0.3 , cosC3 + 0.3 , cosC2 + 0.3, sinC2  + 0.3 ,1.0, 0.8 ));
