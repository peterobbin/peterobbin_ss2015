...
float scope(float x, float y, float scale, float blurriness){
...
void main(){
    ...
    vec3 circleMask = vec3(scope(sinC + 0.3 , cosC + 0.3 , 1.0 / sinT, 0.2 ));
    color = mix(color, vec3(0.5 + 0.5 , 0.0, 0.0), circleMask);

    vec3 circle2Mask = vec3(scope(sinC2 + 0.3 , cosC2 + 0.3 , 1.0 / sinT, 0.2 ));
    vec3 colorC2 = mix(color, vec3(0.0 , 1.0, 0.0), circle2Mask);
    color += colorC2;

    vec3 circle3Mask = vec3(scope(sinC3 + 0.3 , cosC3 + 0.3 , 1.0 / sinT, 0.2 ));
